...
image_paths = []
for i in tqdm(sorted(glob.glob("./finetuned_stable_diffusion_outpainting6/outpainted_images3blended/*"))):
  if detect_lines(i):
...
for m,j in enumerate(tqdm(data)):
    ...
    os.makedirs(output+m+"/masksmrcnn/", exist_ok=True)
    output_image.save(output+m+"/"+m+".png")
    for kj,jk in enumerate(masks_mrcnn):
        Image.fromarray((np.squeeze(jk)*255).astype(np.uint8)).save(output+m+"/masksmrcnn/"+f"{kj}.png")
    ...
    # Sort the masks based on their size (largest to smallest)
    sorted_masks = sorted(selected_masks2, key=lambda mask: -np.sum(mask))

    while sorted_masks:
        current_mask = sorted_masks.pop(0)  # Get the largest remaining mask
        overlapping_masks = [current_mask]  # Start with the current mask

        # Find masks that intersect with the current mask
        i = 0
        while i < len(sorted_masks):
            intersection = np.logical_and(current_mask, sorted_masks[i])
            # print(np.sum(sorted_masks[i]))
            if np.any(intersection):
                # The current mask intersects with the smaller mask
                u = np.unique(intersection, return_counts=True)
                # print(u[1][1]/np.sum(sorted_masks[i]),np.sum(intersection)/np.sum(sorted_masks[i]))
                if u[1][1]/np.sum(sorted_masks[i])>0.9:
                    overlapping_masks.append(sorted_masks.pop(i))
                else:
                    i += 1
            else:
                i += 1

        # Merge the overlapping masks
        merged_mask = merge_masks(overlapping_masks)
        merged_masks.append(merged_mask)

    for ii,jj in enumerate(merged_masks):
        Image.fromarray((jj*255).astype(np.uint8)).save(output+m+"/masks/"+f"{ii}.png")

    prompt_process.plot(
        annotations=torch.tensor(merged_masks),
