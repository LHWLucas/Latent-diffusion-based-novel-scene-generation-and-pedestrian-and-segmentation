...
[os.makedirs(f"./finetuned_stable_diffusion_outpainting3/{n+1}/outpainted_images2blended", exist_ok=True) for n,i in enumerate(prompt_list)]
...
for n,prompt in enumerate(prompt_list):
    ...
    already = [i for i in glob.glob(f"./finetuned_stable_diffusion_outpainting3/{n+1}/outpainted_images2blended/*")]

    for i in tqdm(glob.glob("./stitched_testing/*")[:1]):
        print(i,os.path.basename(i),i.replace("stitched_testing","FRONT_BLEND_TESTING"))
        if i.replace("stitched_testing",f"finetuned_stable_diffusion_outpainting3/{n+1}/outpainted_images2blended") not in already:
            ...
            k2.save(f'./finetuned_stable_diffusion_outpainting3/{n+1}/outpainted_images2blended/{os.path.basename(i)}')


output_size = (1536, 512)

[os.makedirs(f"./finetuned_stable_diffusion_outpainting3/{n+1}/outpainted_images3blended", exist_ok=True) for n,i in enumerate(prompt_list)]

for n,prompt in enumerate(prompt_list):
    already = [i for i in glob.glob(f"./finetuned_stable_diffusion_outpainting3/{n+1}/outpainted_images3blended/*")]

    for i in tqdm(glob.glob(f"./finetuned_stable_diffusion_outpainting3/{n+1}/outpainted_images2blended/*")[:1]):
        ...
        if i.replace("outpainted_images2blended","outpainted_images3blended") not in already:
            ...
            k2.save(f'./finetuned_stable_diffusion_outpainting3/{n+1}/outpainted_images3blended/{os.path.basename(i)}')