...
SAVE_PATH = "./stitched_testing"
os.makedirs(SAVE_PATH, exist_ok=True)
stitch_imgs = glob.glob(SAVE_PATH+"/*")

# load all images
src_imgs = sorted(glob.glob("./FRONT_BLEND_TESTING/*"))
for ii in tqdm(src_imgs):
    print(ii, ii.replace("FRONT_BLEND_TESTING","FRONTRIGHT_BLEND_TESTING"), ii.replace("FRONT_BLEND_TESTING","stitched_testing"))
    print(ii.replace("FRONT_BLEND_TESTING","stitched_testing") not in stitch_imgs)
    if ii.replace("FRONT_BLEND_TESTING","stitched_testing") not in stitch_imgs:
        try:
            imagePaths = [ii, ii.replace("FRONT_BLEND_TESTING","FRONTRIGHT_BLEND_TESTING")]
            images = []
            ...
            while len(images) > 0:
                ...
                if len(matches) >= MIN_MATCH_COUNT:
                    ...
                    cv2.imwrite(f'{ii.replace("FRONT_BLEND_TESTING","stitched_testing")}', ref_img)
                else:
