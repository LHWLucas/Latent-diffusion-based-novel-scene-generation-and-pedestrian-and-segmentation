...
for g,f in tqdm(enumerate(sorted(glob.glob("./data/waymo-od/*.tfrecord"))),desc="creating images from tfrecords"):
  if g==200:
    break

  with open("blend_txt_path.txt", "a") as blend_file:
    blend_file.write(f + "\n")
  ...
  for ind,frame in enumerate(frames_list):
    ...
    for index, image in enumerate(frame.images):
      if open_dataset.CameraName.Name.Name(image.name)=='FRONT':
        ...
        cv2.imwrite(f'./FRONT_BLEND/{count}.png', images[-1])
        count+=1

      if open_dataset.CameraName.Name.Name(image.name)=='FRONT_RIGHT':
        images.append(cv2.resize(cv2.cvtColor(tf.image.decode_jpeg(image.image).numpy(), cv2.COLOR_RGB2BGR),(512,512)))
        cv2.imwrite(f'./FRONTRIGHT_BLEND/{count_right}.png', images[-1])
        count_right+=1
