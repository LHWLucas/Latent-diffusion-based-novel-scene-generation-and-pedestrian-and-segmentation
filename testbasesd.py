...
[os.makedirs(f"./base_stable_diffusion/{n+1}/outpainted_images2blended", exist_ok=True) for n,i in enumerate(prompt_list)]
...
for n,prompt in enumerate(prompt_list):
    ...
    already = [i for i in glob.glob(f"./base_stable_diffusion/{n+1}/outpainted_images2blended/*")]

    for i in tqdm(glob.glob("./stitched_testing/*")):
        print(i,os.path.basename(i),i.replace("stitched_testing","FRONT_BLEND_TESTING"))
        if i.replace("stitched_testing",f"base_stable_diffusion/{n+1}/outpainted_images2blended") not in already:
            ...
            k2.save(f'./base_stable_diffusion/{n+1}/outpainted_images2blended/{os.path.basename(i)}')


output_size = (1536, 512)

[os.makedirs(f"./base_stable_diffusion/{n+1}/outpainted_images3blended", exist_ok=True) for n,i in enumerate(prompt_list)]

for n,prompt in enumerate(prompt_list):
    already = [i for i in glob.glob(f"./base_stable_diffusion/{n+1}/outpainted_images3blended/*")]

    for i in tqdm(glob.glob(f"./base_stable_diffusion/{n+1}/outpainted_images2blended/*")):
        ...
        if i.replace("outpainted_images2blended","outpainted_images3blended") not in already:
            ...
            k2.save(f'./base_stable_diffusion/{n+1}/outpainted_images3blended/{os.path.basename(i)}')